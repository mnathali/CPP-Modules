...
/*   Updated: 2022/08/21 16:44:33 by nikita           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!form.is_signed())
		throw NotSignedException(*this, form);
	else
...
{
	std::cerr << parent.getName() << " couldn’t execute " << parent_form.getName() << " because it is not signed" << std::endl;
	return ("Not sign exception");
}

Bureaucrat::NotSignedException::NotSignedException(const Bureaucrat &parent, const Form &parent_form) : parent(parent), parent_form(parent_form)
{
	std::cout << parent.getName() << " nested class NotSignedException constructor called" << std::endl;
}
